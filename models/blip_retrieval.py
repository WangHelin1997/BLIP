...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.audio_encoder_m = CNNSelfAttention(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            kernel_size=kernel_size,
            padding=padding,
    ...
    def forward(self, audio, caption, alpha, idx):
        with torch.no_grad():
        ...
        audio_atts = torch.ones((audio.shape[0], 1), dtype=torch.long).to(audio.device)        
        audio_feat = F.normalize(self.audio_proj(audio_embeds),dim=-1)    
        
        text = self.tokenizer(caption, padding='max_length', truncation=True, max_length=50, 
        ...
        with torch.no_grad():
            ...
            audio_embeds_m = self.audio_encoder_m(audio, attention_mask) 
            audio_feat_m = F.normalize(self.audio_proj_m(audio_embeds_m),dim=-1)  
            audio_feat_m_all = torch.cat([audio_feat_m.t(),self.audio_queue.clone().detach()],dim=1)                  
            ...
            sim_i2t_m = audio_feat_m @ text_feat_m_all / self.temp  
            sim_t2i_m = text_feat_m @ audio_feat_m_all / self.temp   

            sim_i2t_targets = alpha * F.softmax(sim_i2t_m, dim=1) + (1 - alpha) * sim_targets
            sim_t2i_targets = alpha * F.softmax(sim_t2i_m, dim=1) + (1 - alpha) * sim_targets        

        sim_i2t = audio_feat @ text_feat_m_all / self.temp 
        sim_t2i = text_feat @ audio_feat_m_all / self.temp 
        ...
        self._dequeue_and_enqueue(audio_feat_m, text_feat_m, idxs)        
        ...
        if self.negative_all_rank:    
            ...
            with torch.no_grad():                
                ...
                audio_feat_world = concat_all_gather(audio_feat)
                text_feat_world = concat_all_gather(text_feat)

                sim_i2t = audio_feat @ text_feat_world.t() / self.temp 
                sim_t2i = text_feat @ audio_feat_world.t() / self.temp 
            ...
            # select a negative audio (from all ranks) for each text
            audio_embeds_neg = []    
            for b in range(bs):
            ...
            # select a negative text (from all ranks) for each audio
            ...
            with torch.no_grad():                
                ...
                sim_i2t = audio_feat @ text_feat.t() / self.temp 
                sim_t2i = text_feat @ audio_feat.t() / self.temp 
            ...
            # select a negative audio (from same rank) for each text
            audio_embeds_neg = []    
            for b in range(bs):
                neg_idx = torch.multinomial(weights_t2i[b], 1).item()
            ...
            # select a negative text (from same rank) for each audio    
            text_ids_neg = []
            text_atts_neg = []
            for b in range(bs):
                neg_idx = torch.multinomial(weights_i2t[b], 1).item()
        ...
        audio_embeds_all = torch.cat([audio_embeds_neg,audio_embeds],dim=0)
        audio_atts_all = torch.cat([audio_atts,audio_atts],dim=0)

        output_neg = self.text_encoder(text_ids_all,
                                       attention_mask = text_atts_all,
                                       encoder_hidden_states = audio_embeds_all.unsqueeze(1),
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, audio_feat, text_feat, idxs):
